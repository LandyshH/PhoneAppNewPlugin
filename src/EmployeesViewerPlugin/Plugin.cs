      ...
      while(!command.ToLower().Contains("quit") && !command.ToLower().Equals("q"))
      {
        ...
        switch(command)
        {
          ...
          case "add":
            ...
            var creationEmployeeResult = EmployeesDTO.TryCreateEmployeesDto(name, phone);
            if (!creationEmployeeResult.isSuccess)
            {
              logger.Error(creationEmployeeResult.message);
            }
            else
            {
              employeesList.Add(creationEmployeeResult.employee);
              Console.WriteLine(creationEmployeeResult.message);
            }
            break;
          case "del":
            ...
            } else
            {
              ...
              {
                logger.Error($"Employee with index {indexToDelete} doesn't exist.");
              }
            }
            break;
        }
